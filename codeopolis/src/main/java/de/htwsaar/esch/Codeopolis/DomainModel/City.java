...
public class City extends GameEntity {
    ...
    public void buy(int price, int acres) throws InsufficientResourcesException {
        ...
        if (this.acres + acres >= 0) //Avoid integer overflow
            this.acres += acres;
        else this.acres = Integer.MAX_VALUE;
    }
    ...
    public void plant(int[] acres) throws InsufficientResourcesException, LandOperationException {
        ...
        for (GrainType grainType : GrainType.values()) {
            ...
            if (acres[i] * this.config.getBushelsPerAcre() > this.depot.getFillLevel(grainType))
                throw new InsufficientResourcesException("Not enough bushels to plant " + acres[i] + " acres of grain type " + grainType, acres[i] * this.config.getBushelsPerAcre(), this.depot.getFillLevel(grainType));
        }
        if (acresSum > this.acres)
            throw new LandOperationException("Attempting to plant " + acresSum + " acres, but only " + this.acres + " acres are available.");

        if (acresSum > this.config.getAcrePerResident() * this.residents)
            throw new LandOperationException("Not enough residents to plant " + acresSum + " acres. You can plant " + this.config.getAcrePerResident() + " acres per resident.");
    ...
    public TurnResult runTurn() {
        ...
        int newResidents = 0;
        if (peopleStarvedPercentage < 40) newResidents = (this.residents * this.fortune.nextInt(40)) / 100;
        this.residents -= peopleStarved;
        if (this.residents + newResidents >= 0) //Avoid integer overflow
            this.residents += newResidents;
        else this.residents = Integer.MAX_VALUE;
        ...
        for (int i = 0; i < Game.GrainType.values().length; i++) {
            if (this.planted[i] != null) {
                ...
                if (thisYearsConditions.isDrought()) this.planted[i].drought();
        ...
        Harvest[] thisYearsHarvest = new Harvest[]{
                Harvest.createHarvest(Game.GrainType.BARLEY, harvested[Game.GrainType.BARLEY.ordinal()], this.year),
        ...
        return new TurnResult(this.name, this.year, newResidents, harvested,
                this.residents, this.depot.getBushelsCategorizedByGrainType(),
                peopleStarved, this.acres, ateByRates, peopleStarvedPercentage,
                bushelsDecayed, this.depot.totalCapacity(),
                this.depot.totalCapacity() - this.depot.getTotalFillLevel(), this.depot.toString());
    }
    ...
    public boolean cityExtinct() {
        if (this.residents == 0) return true;
        else return false;
    }
